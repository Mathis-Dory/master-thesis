...
class CustomLoggingCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        if self.n_calls % 100 == 0:
            ...
            payload = env.last_payload  # Access the last payload generated in the environment
        ...
        if self.locals.get("dones", [False])[0] or self.locals.get("truncated", [False])[0]:
            self.episodes += 1
            if self.locals["rewards"][0] > 0:  # Assuming a positive reward indicates success
                self.successes += 1
            success_rate = self.successes / self.episodes if self.episodes > 0 else 0
            self.success_rates.append(success_rate)
            self.success_rates_per_observation.append(self.locals["new_obs"][0])  # Record success rate per observation
        ...
        if self.n_calls % 100000 == 0:
            ...
            print(f"Time taken for the last 100,000 steps: {elapsed_time:.2f} seconds")
            self.model.save(os.path.join(self.save_path, f"model_step_{self.n_calls}"))
    ...
    def _on_training_end(self) -> None:
        total_training_time = time.time() - self.start_time  # Calculate total training time
        ...
        if len(self.success_rates_per_observation) > 0:
            ...
            plt.xticks(ticks=np.arange(0, len(self.success_rates_per_observation), 50000),
                       labels=np.arange(0, len(self.success_rates_per_observation), 50000))

        # Save the plot to a file
        plt.savefig(os.path.join(self.image_dir, "success_rate_per_observation.png"))
        plt.close()
